...
- Python 3.11 이상. lockfile과 `.python-version`(`uv`가 사용)이 가리키는 기준 버전은 3.13이라, 이 버전에서 가장 많이 검증되어 있습니다.
...
> Docker 컨테이너 안에서는 모듈명이 `pwiki.cli`가 아니라 `cli`입니다 — 예:
> `docker compose exec -T pwiki sh -lc 'python -m cli users list'`. 소스 체크아웃에서는
> uv로 실행하세요: `uv run python -m pwiki.cli users list`.
...
Docker 없이 로컬에서 확인하고 싶다면 루트에서 [uv](https://docs.astral.sh/uv/)로 실행합니다 (Python은 `.python-version`으로 고정).
...
| 파일 | 어디에 쓰나 |
|---|---|
| `pyproject.toml` | 의존성의 소스 오브 트루스. uv가 이걸로 `uv.lock`과 `.venv`를 만듭니다. |
| `pwiki/requirements.lock.txt` | Docker 이미지가 실제로 쓰는 고정 버전 런타임 설치(plain `pip`). `pyproject.toml`에서 `uv export`로 재생성합니다. |
| `pwiki/requirements.txt` | uv를 안 쓰고 `pip install -r`만 쓰는 사용자용 느슨한 app 의존성. |
